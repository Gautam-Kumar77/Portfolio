...
const CHIPS = [
  ...
  { label: 'Contact',      q: 'How can I contact Gautam?' },
]
    ...
    <div className={`${styles.overlay} ${open ? styles.open : ''}`} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className={styles.panel} role="dialog" aria-label="AI Terminal">
        ...
        <div className={styles.bar}>
          ...
          <span className={styles.barTitle}>gautam@portfolio ~ AI Assistant</span>
        </div>

        {/* Boot */}
        <div className={styles.boot}>
          <p><span className={styles.bootCyan}>Initializing Gautam's AI Assistant…</span></p>
          <p className={styles.bootMute}>Ask me anything about Gautam — skills, projects, education, or contact.</p>
        ...
        <div className={styles.history} ref={historyRef}>
          {messages.map((m, i) => (
            <div className={`${styles.msg} ${m.role === 'user' ? styles.user : styles.ai}`} key={i}>
              <div className={styles.msgLabel}>{m.role === 'user' ? '▶ you' : '◆ gautam-ai'}</div>
              <div className={styles.msgBody}>{m.text}</div>
            </div>
          ))}
          {loading && (
            <div className={`${styles.msg} ${styles.ai}`}>
              <div className={styles.msgLabel}>◆ gautam-ai</div>
        ...
        <div className={styles.inputRow}>
          <span className={styles.prompt}>gautam@ai $</span>
          <input
            ref={inputRef}
            ...
            placeholder="Ask me anything about Gautam…"
            autoComplete="off"
            spellCheck={false}
          />
